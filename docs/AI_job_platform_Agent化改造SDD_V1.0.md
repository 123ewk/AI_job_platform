...
> 2026-08-29 注记：plan 节点的**真 LLM planner 接入**在 §7 **Step 6.2** 落地（正文早先写"Phase 3 接 `llm_chat_functions`"是欠账——Phase 3/4 实际只接了工具，缺省 planner 仍是骨架 echo，DoD #1 未成立）。接缝契约 `planner(messages, tool_schemas) -> decision` 不变；缺省链为「有 AI key 用真 LLM，无 key 回退 echo 保冒烟」。
...
### Phase 6 · 真 LLM 接入与收尾（2026-08-29 调整：新增 6.2 真 LLM planner 接入，原 6.2 dashboard 重编号为 6.3）

- **6.1 文档** ✅ 已完成（2026-08-29）：①**README 加「🤖 Agent 对话层」章节**——自然语言操控入口/执行模式（audit/autonomous）/6 工具清单/安全与工程护栏（审批门·后台任务·崩溃恢复·DRY_RUN·注入防御链·WS 进度）/agent API 端点表 + 指向 `docs/AGENT_USAGE.md`；头部加 Agent 徽章、目录加条目、项目结构树补 `agent/` 与 `docs/`。②**TECHNICAL_ANALYSIS 并发模型补 FlowLock**——`§8.4` 重写为"三类浏览器消费者 + pw 单线程池 + FlowLock 显式互斥"（Agent 工具持有/监控循环 `flow_lock.locked()` 跳本轮/Agent 阻塞排队，含互斥示意图），新增 `§8.5 TaskExecutor 自有事件循环线程`（任意线程 `run_coroutine_threadsafe` 提交、逐岗位进度、单位间停止/熔断、DB 每线程独立连接）；`§2` 文件表补 10 个 agent 模块、`§6.1` 监控循环第 2 步补 `flow_lock.locked()`、`§9.1` 新增链路 E（Agent 自然语言操控全数据流）、`§10` 技术表补 FlowLock/TaskExecutor 两行、`§11.3` 补 `browser_sync_lock → FlowLock` 升级说明。③**新建 `docs/AGENT_USAGE.md`**（Agent 对话层使用指南，13 节）：一句话概括/curl 快速开始/执行模式/工具清单（入参+读写分类+JobStatus 词汇）/审批门 decide 流程图/后台任务全链路（状态机·进度·手动刹车·熔断）/崩溃恢复与结果未知确认门/DRY_RUN 演练/安全边界（白名单·敏感键·安全开关·注入防御链·脱敏）/WS 事件表/HTTP API 参考/典型对话场景/4 张 agent 表 + checkpoint 落库 + 附并发模型速览。门禁：纯文档改动不新增测试；pytest **154 passed / 38 skipped** 与 ruff All passed 回归不变（无代码变更，跑全量确认无回归）。范围：Phase 6 文档收口；dashboard 对话面板为可选，可后置（2026-08-29 重编号为 6.3）。
- **6.2 真 LLM planner 接入** ✅ 已完成（2026-08-29；补 §4.1「plan 接缝 Phase 3 接 `llm_chat_functions`」的欠账——Phase 3/4 只接了工具，`agent/service.py` 缺省 planner 仍是 `echo_planner_factory` 确定性假 planner，用户自然语言不会被理解，DoD #1 不成立。本步把 Step 2.2 的 `llm_chat_functions` 真正接上决策图，**不改 graph 决策契约、不改任何工具、不改审批门/防御链行为**）：
  - **①新增 `agent/planner.py`**（真 LLM 决策器，唯一职责：把 graph 的 planner 接缝接到 LLM）：
    - 契约不变：仍实现 `planner(messages=trace, tool_schemas) -> decision`，decision 三态与 §4.1 相同（tool / ask_user / report）——graph/service/registry 零结构改动；
    - **system prompt 组装（L0 保持）**：`PLANNER_SYSTEM_PROMPT = defense.SYSTEM_PROMPT + "\n\n" + OPERATIONAL_RULES`——前者是 5.2 的服务端常量（安全声明原样前置），后者为本步新增常量（§4.2/§4.4 硬规则的 LLM 可读版）：打招呼前必须先 `query_jobs(ungreeted=true)` 查库存、有库存先投库存（send_greetings）再搜新（search_jobs，max_pages≤3）；JobStatus 词汇表（discovered/pending/greeted/applied/replied/interview/filtered/unknown）；缺必填信息（城市/数量/关键词）调 `ask_user` 禁止编默认值、一次反问仍缺 → 带默认值的确认式问题（§4.4）；动手前可先 `get_progress` 看今日额度；汇报用中文、简洁、带数字结果；
    - **ask_user 伪工具**：§4.2 定义 ask_user 为工具，但 graph 把它实现为决策路由（非 registry 执行件）——planner 调 LLM 前把 `ask_user` 的 OpenAI schema（参数 `{question}`）**追加进本次调用的 tool_schemas 副本**（只进 LLM 声明、不进 ToolRegistry 白名单）；LLM 调它 → decision `{"action": "ask_user", "question": arguments["question"]}`；
    - **trace → OpenAI 规范消息映射**：trace 的 `{role:user}` → user 原样；`{role:assistant, decision}` → assistant 消息——action=tool 的决策转规范 `tool_calls`（id=`call_N`，arguments=JSON 字符串），ask_user/report 决策转 assistant 文本（decision JSON 字符串）；`{role:tool, content}` → `{"role":"tool","tool_call_id":<最近未配对的 call_N>,"content":<untrusted 载荷>}`（OpenAI 兼容服务端要求 tool 消息必须配对前面的 assistant tool_calls，裸 role:tool 不合规；审批拒绝回灌消息同此映射）；
    - **返回解析**：`message.tool_calls` 非空 → 只取第一个（多调用忽略并 WARNING 日志，决策图一次执行一个工具）：name=="ask_user" → ask_user 决策；否则 `{"action":"tool","name":...,"arguments":json.loads(arguments or "{}")}`（arguments 非法 JSON → `{}`，交给工具 Pydantic L3 校验回 error dict 自纠，不抛）；tool_calls 为空 → `{"action":"report","content": message.content or "（无内容）"}`；
    - **失败降级**：`llm_chat_functions` 抛 Exception（网络/HTTP/裸 key）→ ERROR 日志 + 返回 report 决策（"LLM 调用失败：{原因}"），回合诚实收尾不 500；
    - **key 探测**：`llm_planner_factory(user_input) -> planner|None`——`_load_ai_config()` 无 api_key 返回 None（复用 Step 0.2 env 优先、settings 兜底链）。
  - **②service 缺省链切换**：`AgentService._invoke` 缺省 planner 由 `echo_planner_factory` 改为 `default_planner_factory`（= `llm_planner_factory(user_input) or echo_planner_factory(user_input)`）——**有 key 用真 LLM、无 key 回退 echo**（2.4「无 key 可冒烟」承诺不破）；`make_planner` 注入优先级不变（已核对 test_agent_api/approval_gate/dry_run/tools 全部显式传 make_planner，零影响）；echo 工具保留 default_registry（回归护栏）。
  - **③graph 唯一行为改动——未注册工具 error 回灌**：真 LLM 可能幻觉出不存在的工具名，`_execute_tool` 对 `registry.get(name) is None` 由「抛 KeyError（整个回合 500）」改为「tool_result={"error": "未注册的工具：{name}", "allowed": registry.names()} 落库回灌」（与工具 Pydantic 校验失败同模式，LLM 可自纠，§3.1 先例）；已核对既有测试无 KeyError 断言。
  - **④单测 `tests/test_agent_planner.py`**（mock `llm_chat_functions` 与 `_load_ai_config`，不打真 HTTP/不碰浏览器）：tool_calls→tool 决策（arguments JSON 解析）、空 tool_calls content→report、ask_user 伪工具→ask_user 决策、多 tool_calls 只取第一个、arguments 非法 JSON→{}、LLM 异常→report 降级、无 key→factory 返回 None、trace 映射（user/assistant/tool→规范 OpenAI 消息 + tool_call_id 配对）、system prompt 含 SYSTEM_PROMPT 与 OPERATIONAL_RULES 关键规则、service 缺省组合（无 key 用 echo / 有 key 用真 LLM 端到端 mock）、graph 未注册工具 error 回灌自纠 E2E（第一轮幻觉工具名→error 回灌→第二轮 report 收尾，回合不抛）。
  - **⑤门禁**：ruff 全绿；pytest 全量绿（154 基线 + 新增）；真实冒烟：配 .env key 后经 AgentService 冒烟「帮我看看有没有没打招呼的岗位」→ 返回真 LLM 决策的 query_jobs 执行与汇报（audit 下写操作仍走审批）。
  - **实施记录（随 Step 6.2 提交）**：①`agent/planner.py` 落地（`llm_planner_factory` key 探测无 key 返回 None / `default_planner_factory` 有 key 真 LLM 无 key 回退 echo / `parse_llm_message` 三态解析——tool_calls 只取第一个多则 WARNING、ask_user 伪工具路由、arguments 非法 JSON 或非 dict → `{}` 走 L3 自纠 / `trace_to_openai_messages` 规范映射——assistant tool 决策生成 `call_N` tool_calls、role:tool 结果按 tool_call_id 配对、ask_user/report 决策转 assistant 文本 / `PLANNER_SYSTEM_PROMPT` = `defense.SYSTEM_PROMPT` + `OPERATIONAL_RULES` 六条工作规则 + `ASK_USER_TOOL_SCHEMA` 伪工具只进 LLM 声明、temperature=0.2）。②service 缺省链切换（`make_planner` 注入优先不变，既有测试零改动全绿）。③graph `_execute_tool` 未注册工具 KeyError → `{"error", "allowed"}` 回灌。④**实现中发现并修复 DeepSeek 思考模式坑**：`deepseek-v4-pro` 回合内多轮调用要求把上轮 `reasoning_content` 传回（实测 replan 必 400 "The reasoning_content in the thinking mode must be passed back to the API"；reasoning 在 graph trace 中不保留），而决策循环逐工具重规划至多 12 次必须低时延——`llm_chat_functions` 加通用 `extra_body` 逃生口（默认 None 行为不变），planner 经 `_extra_body` 对 DeepSeek 端点发 `DISABLE_THINKING_BODY={"thinking":{"type":"disabled"}}` 关思考（实测 round1 无 reasoning、round2 200），其他端点不发未知字段。⑤单测 `tests/test_agent_planner.py` 17 个（比 spec ④ 多 1 个 extra_body 供应商分支）。⑥门禁全绿：ruff All passed；pytest **171 passed / 38 skipped**（较 5.3 的 154 新增 17）。⑦真实冒烟（DeepSeek deepseek-v4-pro + 只读 query_jobs/get_progress 注册表，不碰浏览器/写路径）：首轮模型**自主**按规则决策 `query_jobs(ungreeted=true)`（OPERATIONAL_RULES 生效）、工具回灌后 replan 生成含表格的中文汇报（2 条库存）、status=completed、transcript 四步 plan→execute→plan→report；首轮 400 复现→关思考修复→完整通过。⑧`docs/AGENT_USAGE.md` 生产 Planner 说明同步（缺省链/思考模式/make_planner 注入）。
- **6.3 dashboard 集成** ✅ 已完成（2026-08-29；原 6.2 重编号）——`static/dashboard.html` 加「🤖 Agent」对话面板（最简实现，原生 JS + fetch + WebSocket，不引前端框架/构建链，风格随既有 dashboard）：
  - **对话区**：消息列表（用户/Agent 气泡）+ 输入框 + 发送按钮 + 执行模式下拉（audit 默认/autonomous，§4.3）→ `POST /api/agent/chat`；report / ask_user_question 渲染进气泡；`status=pending_approval` 时渲染审批卡片（工具名 + 掩码参数快照 + 批准/拒绝按钮 → `POST /api/agent/approvals/{id}/decide`，decide 返回续渲染）；
  - **步骤流**：`/ws/agent` 的 `agent_step`（plan/execute/approval/report/ask_user）渲染为步骤时间线，`agent_chat_done` 标记回合结束（Step 2.4/5.1 事件契约）；
  - **后台任务卡片**：`agent_task_progress` / `agent_task_done` 事件驱动（进度 done/total + status，§4.5），卡片「停止」按钮 → `POST /api/agent/tasks/{id}/stop`（刹车柄只在用户手里，§4.2/§4.5）；
  - **门禁**：pytest/ruff 回归不变；新增轻量测试 `tests/test_dashboard_agent_panel.py` 断言 dashboard.html 含 chat/decide/stop 端点与 /ws/agent 引用（防回归漂移）；手工冒烟：浏览器对话一轮、audit 审批批准一次、后台任务进度与停止；
  - 不影响后端 DoD。
  - **实施记录（随 Step 6.3 提交）**：①侧边栏新 tab「🤖 Agent 对话」+ `#tab-agent` 主体（复用聊天页 `chat-pane`/`msg-row`/`msg-bubble` 气泡样式，Agent 气泡居左 ai-av、用户居右）；对话区含消息列表 + 输入框（Enter 发送/Shift+Enter 换行）+ **执行模式下拉**（audit 默认；切 autonomous 弹 confirm 二次确认后仍可回退 audit）→ `POST /api/agent/chat`（body 含 `thread_id`/`execution_mode`，thread_id 页面级生成一次，ask_user 靠同 thread_id 续会话）。②report/ask_user 渲染进气泡；`status=pending_approval` 渲染**审批卡片**（🔒 工具名 + 掩码参数 JSON + 批准/拒绝按钮 → `POST /api/agent/approvals/{id}/decide`，点击后卡片按钮全禁用防重复 decide，409/404 的 `detail` 也渲染进气泡）；decide 返回与 chat 响应共用 `renderAgentTurnResult`（可再次挂起新审批）。③**步骤时间线**：独立 `WebSocket /ws/agent`（断线 3s 重连，与主 `/ws` 并行常连），`agent_step` 按 kind 渲染为紧凑 mono 行（⏺ plan→决策 query_jobs / execute→执行+掩码入参 / approval→挂起 / ask_user / report），`agent_chat_done` 仅标记回合结束（清思考中指示器——内容渲染以 HTTP 响应为唯一真源，避免双渲染）。④**后台任务卡片**：`agent_task_progress`/`agent_task_done` 驱动（进度条 done/total + 状态徽章：完成/已停止/失败/连续失败熔断），进行中卡片带「停止」按钮 → `POST /api/agent/tasks/{id}/stop`（返回 message 进 toast，任务终态后 8s 自动移除卡片）。⑤轻量测试 `tests/test_dashboard_agent_panel.py` 3 个（tab+三组端点 / WS+五 kind 事件契约+StepKind 五步骤 / 模式下拉+approve/reject 决策契约），防回归漂移；JS 经 Node `new Function` 语法校验。⑥门禁：ruff All passed；pytest **174 passed / 38 skipped**（171 基线 + 3 新增）。⑦真实冒烟（boss_app 起真实端口，无浏览器、零写路径）：GET / 面板在 → `/ws/agent` agent_connected → 只读「查库存没打招呼岗位」真 LLM 回合（WS 步骤流 plan→execute→plan→report + chat_done，汇报 492 条库存）→ audit 写意图「给第一个待投岗位打招呼」→ `pending_approval`（send_greetings 挂起）→ decide reject → 诚实改道汇报（无任何发送）→ 重复 decide 409 幂等门——面板消费的全部事件/响应契约端到端验证通过。
...
- 2026-08-30 V1.2.27（用户实测 hotfix，随修复提交）：①**多轮探索打满递归上限 → 裸 500**：用户实测「获取杭州实习生岗位」，LLM 查库存(空)→search_jobs(3页)→再查库存→去掉 ungreeted 再查，第 5 轮 plan 触发 LangGraph GraphRecursionError（DEFAULT_RECURSION_LIMIT=12 按 plan+execute 各计一步，一轮「查→搜→核」就要 8-10 步，12 必误熔断）→ 异常未接住变成纯文本 500，前端 r.json() 报 "Unexpected token 'I'"。三层修复：上限 12→30（语义澄清：熔断按**轮数**不按时长，慢工具只占 2 步不额外烧预算）；service._build_and_run 捕 GraphRecursionError 收尾成正常 report 回合（步骤已实时经 WS 推送，过程不丢）；api/chat 兜底 BLE001 → JSONResponse(500) 保 JSON 契约 + 前端 sendAgentChat 对非 JSON 响应兜底渲染。②**search_jobs 透出 job_type 求职类型参数**：底层 BossScraper.search 早已支持 jobType URL 参数（1901 全职/1902 实习/1903 兼职），工具层此前没暴露——用户说「实习」时 LLM 只能把「实习生」塞进 keyword，类型不可控还空耗查询轮次。SearchJobsParams.job_type 白名单归一（中英文/数字别名 → 4 位 code，非法值 ValidationError 走 L3 自纠），_search_page_n 翻页 URL 透传 jobType（丢了则第 2 页起筛选静默失效），返回体带 job_type，注册表描述 + OPERATIONAL_RULES 第 1 条同步告知 LLM。新增测试 5 个（透传+翻页保参 / 别名归一校验 / 递归熔断友好收尾 / 上限=30 守卫 / chat 端点 JSON 兜底），门禁：ruff All passed；pytest **190 passed / 38 skipped**（185+5）。
- 2026-08-29 V1.2.26（用户实测三个 hotfix，随修复提交）：①**汇报格式不稳定（气泡直出原始 JSON）**：`trace_to_openai_messages` 把 ask_user/report 决策序列化成 decision JSON 文本回灌，模型照猫画虎用纯文本输出决策，`parse_llm_message` 无 tool_calls 时把 content 原样当 report content——用户实测气泡显示 `{"action":"report","content":"…"}` 原文。双端修复：**源头**——trace 回灌改纯文本正文（异常形状兜底仍回 JSON，决策轨迹以 agent_steps 落库为准）；**兜底**——新增 `_unwrap_text_decision`：content 为决策 JSON 文本时解包 report/ask_user 取正文（支持 ```json 围栏），action=tool 形状**不执行**（执行决策只认 tool_calls 结构化通道，白名单/审批门接缝不变）原文收尾；OPERATIONAL_RULES 第 5 条补"汇报/反问直接给正文，不要 JSON 包装"。②**浏览器工具恒报"浏览器未启动"（影子模块副本）**：boss_app 以 `python boss_app.py` 启动时模块名是 __main__，agent 包 `from boss_app import automation` 会把文件再执行一遍生成影子副本——`/api/system/start` 更新的是 __main__.automation，工具读到的影子值恒 None（用户实测：浏览器已启动仍报未启动，"明明登录了"）。修复：`agent/tools.py` 新增 `live_boss_app()`（__main__.__file__ 是 boss_app.py → 优先 __main__，否则用 boss_app 本尊），`_default_get_automation`/`_default_paused`/`_default_executor`/`_default_pw_runner` 与 `service._runtime_paused` 全部改经它取（app.state.agent_executor / monitor_paused 同类隐患一并收口）；**其中 `_run_pw` 尤其关键**——它闭包持有各自副本的 `_playwright_executor`（pw 单线程池），从影子副本拿会让浏览器 start 与工具操作落在不同 OS 线程，Playwright sync 对象绑线程直接 greenlet "Cannot switch to a different thread"（实测撞上）；新增 `test_default_loader_reads_live_module_not_shadow_copy` + `test_default_pw_runner_uses_live_module_executor`。③"浏览器未启动"话术澄清（扫码登录≠浏览器实例运行）+ **dashboard 气泡最小 Markdown 渲染** `agentMd`（esc 后 `**加粗**`/`` `代码` ``/换行）。④`parse_llm_message` 文本型决策解包新增测试 5 个（report 解包 / 围栏+转义 / ask_user 解包 / tool 形状不执行 / 纯文本不受影响，红→绿）+ trace 映射断言改正文 + `test_agent_bubble_renders_inline_markdown`。门禁：ruff All passed；pytest **184 passed / 38 skipped**（183+1）。
- 2026-08-29 V1.2.25（用户实测两个 hotfix，随修复提交）：①**dashboard TDZ 中断**（0bb73a8 引入回归）：顶层启动行 `agentWsConnect()` 执行时 `agentWs` 等 let/const 尚未声明（声明在脚本 3087 行），首行踩 TDZ 抛 ReferenceError 中断整个顶层脚本——其后声明的 `appAllJobs`/`PLATFORMS`/`agentWs`/`agentBusy` 全部未初始化，岗位列表空白、筛选无响应、Agent 恒显未连接、点发送零请求（fetch 前即抛错）。修复：启动行移至脚本末尾（所有顶层声明之后）+ `tests/test_dashboard_agent_panel.py` 回归断言启动调用在 `let agentWs`/`let appCurrentPage` 声明之后。②**Agent 多轮输入被吞**（Step 2.3 起 `_plan` 注入时机缺陷）：L0 注入仅在 trace 无 user 消息时执行（为回合内 replan 幂等设计），同 thread 第二轮 invoke 的新 `user_input`（追问 / ask_user 答复）进不了 planner 视野，模型拿首轮上下文作答——用户实测"问不同的问题答案不变"。修复：`agent/graph.py` `_plan` 改为输入不在历史 user 消息中 → 首轮前置 / 后续轮追加 trace 末尾（回合内 replan 输入未变依旧幂等跳过）；新增 `test_followup_turn_new_question_reaches_planner` + `test_ask_user_answer_reaches_planner_next_turn`（红→绿）。门禁：ruff All passed；pytest **177 passed / 38 skipped**（175+2）。
- 2026-08-29 V1.2.24（随 Step 6.3 提交）：dashboard「🤖 Agent」对话面板（Phase 6 收官，SDD 全步骤完成）。①`static/dashboard.html`：侧边栏新 tab + `#tab-agent`（复用聊天页气泡样式零新框架）；对话区（消息列表 + 输入框 + 执行模式下拉 audit 默认，autonomous 切换 confirm 二次确认）→ `POST /api/agent/chat`；审批卡片（工具名 + 掩码参数 + 批准/拒绝 → `POST /api/agent/approvals/{id}/decide`，点击后禁用防重复，decide 与 chat 共用结果渲染，可链式再挂起）；WS 步骤时间线（独立 `/ws/agent` 断线重连，agent_step 按 kind 渲染 mono 行，agent_chat_done 仅标回合结束、内容以 HTTP 响应为唯一真源防双渲染）；后台任务卡片（agent_task_progress/done 驱动进度条 + 停止按钮 → `POST /api/agent/tasks/{id}/stop`，终态 8s 自动移除）。②轻量测试 `tests/test_dashboard_agent_panel.py` 3 个（端点/WS 事件契约/StepKind 五步骤/模式与 decide 契约断言）。门禁：ruff All passed；pytest **174 passed / 38 skipped**（171+3）。真实冒烟（boss_app 真端口、无浏览器零写）：面板在页 + agent_connected + 只读真 LLM 回合（步骤流 plan→execute→plan→report）+ audit 写意图挂起 send_greetings → reject 改道汇报 + 重复 decide 409——面板消费契约端到端全通过。`docs/AGENT_USAGE.md` 补 dashboard 面板为 UI 入口一行。
- 2026-08-29 V1.2.23（随 Step 6.2 提交）：真 LLM planner 接入（§4.1 欠账收口，DoD #1 成立）。①**新增 `agent/planner.py`**：`llm_planner_factory`（`_load_ai_config` key 探测，无 key 返回 None）/ `default_planner_factory`（有 key 真 LLM、无 key 回退 echo，2.4 冒烟承诺不破）/ `parse_llm_message`（tool_calls 只取第一个、ask_user 伪工具→ask_user 决策、arguments 非法 JSON→{} 走 L3 自纠、空 tool_calls content→report）/ `trace_to_openai_messages`（assistant tool 决策→规范 `tool_calls` id=call_N、role:tool 结果按 tool_call_id 配对、ask_user/report 决策→assistant 文本；审批拒绝回灌同映射）/ `PLANNER_SYSTEM_PROMPT` = `defense.SYSTEM_PROMPT`（L0）+ `OPERATIONAL_RULES`（先查库存再投库存再搜新、JobStatus 词汇、ask_user 反问禁编默认值、get_progress 额度、中文汇报、error 自纠）+ `ASK_USER_TOOL_SCHEMA`（只进 LLM 声明不进 registry 白名单）、temperature=0.2。②`agent/service.py` 缺省链切换（`make_planner` 注入优先不变）。③`agent/graph.py` `_execute_tool` 未注册工具 KeyError → `{"error","allowed"}` error 回灌自纠（真 LLM 幻觉工具名不再炸回合）。④`interview/llm_client.py` `llm_chat_functions` 加 `extra_body` 逃生口（默认 None 行为不变）。⑤**修复 DeepSeek 思考模式 400**：deepseek-v4-pro 多轮要求回传 `reasoning_content`（replan 必 400），planner 经 `_extra_body` 对 DeepSeek 端点发 `{"thinking":{"type":"disabled"}}` 关思考，其他端点不发未知字段。⑥单测 `tests/test_agent_planner.py` 17 个。门禁全绿：ruff All passed；pytest **171 passed / 38 skipped**（较 5.3 的 154 新增 17）。真实冒烟：模型自主 `query_jobs(ungreeted=true)`→回灌 replan→表格汇报 2 条库存→completed。`docs/AGENT_USAGE.md` 生产 Planner 说明同步。
- 2026-08-29 V1.2.22（文档补全，先行于 Step 6.2 开发，纯文档 commit）：核对代码发现 §4.1「plan 接缝 Phase 3 接 llm_chat_functions」是欠账——Phase 3/4 只接了工具，`agent/service.py:214` 缺省 planner 仍为 `echo_planner_factory` 假 planner，自然语言不被理解，DoD #1「自然语言→全链路」未成立。①**新增 Step 6.2「真 LLM planner 接入」**（spec 化：`agent/planner.py` 三态 decision 契约不变、`defense.SYSTEM_PROMPT + OPERATIONAL_RULES` 组装、ask_user 伪工具声明、trace→OpenAI 规范消息映射含 tool_call_id 配对、tool_calls 解析与失败降级、无 key 回退 echo 缺省链、graph 未注册工具 KeyError 改 error 回灌、单测清单）。②**原 6.2 dashboard 集成重编号为 6.3** 并细化 spec（对话区/审批卡片/WS 步骤流/后台任务卡片+停止按钮/轻量回归测试）；Phase 6 标题改「真 LLM 接入与收尾」，6.1 范围行同步改引用。③§4.1 加注记指向 Step 6.2。无代码改动。
