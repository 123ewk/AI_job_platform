...
- **6.2 真 LLM planner 接入**（2026-08-29 补入文档；补 §4.1「plan 接缝 Phase 3 接 `llm_chat_functions`」的欠账——Phase 3/4 只接了工具，`agent/service.py` 缺省 planner 仍是 `echo_planner_factory` 确定性假 planner，用户自然语言不会被理解，DoD #1 不成立。本步把 Step 2.2 的 `llm_chat_functions` 真正接上决策图，**不改 graph 决策契约、不改任何工具、不改审批门/防御链行为**）：
